...
class BackupFormat:
    ...
    def __init__(self, name: str, compress_program: str = "", suffix: str = ""):
        self.name = name
    ...
    @staticmethod
    def from_dict(d: dict) -> "BackupFormat":
        return BackupFormat(**d)

    @staticmethod
    def from_legacy(name: str) -> "BackupFormat":
        comp_suffix = BackupFormat.to_comp_suffix.get(name)
        return BackupFormat(name, comp_suffix[0], comp_suffix[1])


def get_backup_format() -> BackupFormat:
    return BackupFormat(config.backup_format, config.compress_program, config.suffix)


def tr(translation_key: str, *args) -> RTextMCDRTranslation:
    return ServerInterface.get_instance().rtr(
        "quick_backup_multi.{}".format(translation_key), *args
    )
...
def copy_file_fast(src_path: str, dst_path: str) -> str:
    ...
    try:
        ...
        server_inst.logger.warning(
            "copy_file_range {} -> {} failed ({}), retrying with shutil.copy".format(
                src_path, dst_path, e
            )
        )
...
def copy_worlds(
    src: str,
    dst: str,
    intent: CopyWorldIntent,
    *,
    backup_format: Optional[BackupFormat] = None,
):
        ...
        for world in config.world_names:
            ...
            if os.path.isdir(src_path):
                shutil.copytree(
                    src_path,
                    dst_path,
                    ignore=lambda path, files: set(
                        filter(config.is_file_ignored, files)
                    ),
                    copy_function=copy_file_fast,
                )
    ...
    elif backup_format.name == "tar":
        tar_args = ["tar"]
        if backup_format.compress_program != "":
            tar_args += ["-I", backup_format.compress_program]

        if intent == CopyWorldIntent.restore:
            tar_path = os.path.join(src, get_backup_file_name(backup_format))

            tar_args += ["-C", dst, "-xf", tar_path]
            ...
            server_inst.logger.info("storing {} -> {}".format(src, tar_path))
            subprocess.run(tar_args)
...
def create_slot_info(comment: Optional[str]) -> dict:
    slot_info = {
        ...
        "backup_format": vars(get_backup_format()),
    }
...
def clean_up_slot_1():
    ...
    if target_slot_idx is not None:
        ...
        server_inst.logger.info(
            "Slot {} ({}) is deleted to provide spaces for the incoming backup".format(
...
def _create_backup(source: CommandSource, comment: Optional[str]):
    try:
        ...
        while True:
            ...
            if plugin_unloaded:
                print_message(
                    source, tr("create_backup.abort.plugin_unload"), tell=False
                )
                return
        ...
        print_message(
            source,
            tr("create_backup.success", round(end_time - start_time, 1)),
            tell=False,
        )
...
def restore_backup(source: CommandSource, slot: int):
    ...
    print_message(
        source,
        tr("restore_backup.echo_action", slot, format_slot_info(info_dict=slot_info)),
        tell=False,
    )
    print_message(
        source,
        command_run(
            tr("restore_backup.confirm_hint", Prefix),
            tr("restore_backup.confirm_hover"),
            "{0} confirm".format(Prefix),
        )
        + ", "
        + command_run(
            tr("restore_backup.abort_hint", Prefix),
            tr("restore_backup.abort_hover"),
...
def _do_restore_backup(source: CommandSource, slot: int):
    try:
            ...
            print_message(
                ...
                command_run(
                    tr(
                        ...
                        10 - countdown,
                        slot,
                        format_slot_info(info_dict=slot_info),
                    ),
        ...
        backup_format = slot_info.get("backup_format")
        if type(backup_format) == dict:
            backup_format = BackupFormat.from_dict(backup_format)
        else:
            backup_format = BackupFormat.from_legacy(backup_format)

        server_inst.logger.info(
            "Restore backup {} (mode={})".format(slot_folder, backup_format.name)
        )
        copy_worlds(
            slot_folder,
            config.server_path,
            CopyWorldIntent.restore,
        ...
        source.get_server().dispatch_event(
            RESTORE_DONE_EVENT, (source, slot, slot_info)
        )  # async dispatch
...
def list_backup(source: CommandSource, size_display: bool = None):
    ...
    for i in range(get_slot_count()):
        ...
        text = RTextList(
            RText(tr("list_backup.slot.header", slot_idx)).h(
                tr(
                    "list_backup.slot.protection",
                    format_protection_time(
                        config.slots[slot_idx - 1].delete_protection
                    ),
                )
            ),
            " ",
        ...
        if formatted_slot_info is not None:
            text += RTextList(
                ...
                .h(tr("list_backup.slot.restore", slot_idx))
                .c(RAction.run_command, f"{Prefix} back {slot_idx}"),
                RText("[×] ", color=RColor.red)
                .h(tr("list_backup.slot.delete", slot_idx))
                .c(RAction.suggest_command, f"{Prefix} del {slot_idx}"),
            )
            if size_display:
                text += RText(format_dir_size(dir_size) + " ", RColor.dark_green).h(
                    BackupFormat.of(slot_info.get("backup_format")).name
                )
    ...
    if size_display:
        print_message(
            source,
            tr("list_backup.total_space", format_dir_size(total_backup_size)),
            prefix="",
        )
...
def register_command(server: PluginServerInterface):
    ...
    def get_slot_node():
        return (
            Integer("slot")
            .requires(lambda src, ctx: 1 <= ctx["slot"] <= get_slot_count())
            .on_error(
                RequirementNotMet,
                lambda src: src.reply(tr("command.wrong_slot")),
                handled=True,
            )
        )
        ...
        .then(
            ...
            .then(
                GreedyText("comment").runs(
                    lambda src, ctx: create_backup(src, ctx["comment"])
                )
            )
        )
        .then(
            ...
            .then(
                get_slot_node().runs(lambda src, ctx: restore_backup(src, ctx["slot"]))
            )
        )
        .then(
            get_literal_node("del").then(
                get_slot_node().runs(lambda src, ctx: delete_backup(src, ctx["slot"]))
            )
        )
        .then(
            get_literal_node("rename").then(
                get_slot_node().then(
                    GreedyText("comment").runs(
                        lambda src, ctx: rename_backup(src, ctx["slot"], ctx["comment"])
                    )
                )
            )
        )
...
def load_config(server: ServerInterface, source: CommandSource or None = None):
    ...
    for i in range(get_slot_count()):
        ...
        if this < 0:
            ...
            server.logger.warning(
                "Slot {} has a delete protection time smaller than the former one".format(
                    i + 1
                )
            )
        last = this
...
def register_event_listeners(server: PluginServerInterface):
    server.register_event_listener(
        TRIGGER_BACKUP_EVENT,
        lambda svr, source, comment: _create_backup(source, comment),
    )
    server.register_event_listener(
        TRIGGER_RESTORE_EVENT,
        lambda svr, source, slot: _do_restore_backup(source, slot),
    )
...
def on_load(server: PluginServerInterface, old):
    ...
    server.register_help_message(
        Prefix,
        command_run(
            tr("register.summory_help", get_slot_count()),
            tr("register.show_help"),
            Prefix,
        ),
    )
